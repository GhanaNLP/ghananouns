...
def translate_batch_to_southern_dagaare(nouns):
    """
    Translate a batch of nouns to Southern Dagaare using Gemini API
    """
    try:
        # Create the model
        ...
        prompt = f"""Translate these list of nouns extracted from Ghanaian news articles to Southern Dagaare. Return ONLY the translations in the same numbered format, one per line. Do not add any explanations or extra text.
...
def main():
    ...
    output_file = "nouns_with_southern_dagaare_translations.csv"
    ...
    if Path(output_file).exists():
        ...
        # Check if southern_dagaare_translation column exists
        if 'southern_dagaare_translation' in existing_df.columns:
            # Find the last non-null translation
            non_null_mask = existing_df['southern_dagaare_translation'].notna()
            if non_null_mask.any():
                ...
                df['southern_dagaare_translation'] = None
        else:
    ...
    for batch_num in range(total_batches):
        ...
        # Translate the batch
        translations = translate_batch_to_southern_dagaare(batch_nouns)
        
        # Assign translations back to dataframe
        for i, translation in enumerate(translations):
            df.loc[batch_start + i, 'southern_dagaare_translation'] = translation
    ...
    successful = df['southern_dagaare_translation'].notna().sum()
