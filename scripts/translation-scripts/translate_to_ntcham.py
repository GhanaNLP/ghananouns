...
def translate_batch_to_ntcham(nouns):
    """
    Translate a batch of nouns to Ntcham using Gemini API
    """
    try:
        # Create the model
        ...
        prompt = f"""Translate these list of nouns extracted from Ghanaian news articles to Ntcham. Return ONLY the translations in the same numbered format, one per line. Do not add any explanations or extra text.
...
def main():
    ...
    output_file = "nouns_with_ntcham_translations.csv"
    ...
    if Path(output_file).exists():
        ...
        # Check if ntcham_translation column exists
        if 'ntcham_translation' in existing_df.columns:
            # Find the last non-null translation
            non_null_mask = existing_df['ntcham_translation'].notna()
    ...
    for batch_num in range(total_batches):
        ...
        # Translate the batch
        translations = translate_batch_to_ntcham(batch_nouns)
        
        # Assign translations back to dataframe
        for i, translation in enumerate(translations):
            df.loc[batch_start + i, 'ntcham_translation'] = translation
    ...
    successful = df['ntcham_translation'].notna().sum()
