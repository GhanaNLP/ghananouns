...
def translate_batch_to_southern_birifor(nouns):
    """
    Translate a batch of nouns to Southern Birifor using Gemini API
    """
    try:
        # Create the model
        ...
        prompt = f"""Translate these list of nouns extracted from Ghanaian news articles to Southern Birifor. Return ONLY the translations in the same numbered format, one per line. Do not add any explanations or extra text.
...
def main():
    ...
    output_file = "nouns_with_southern_birifor_translations.csv"
    ...
    if Path(output_file).exists():
        ...
        # Check if southern_birifor_translation column exists
        if 'southern_birifor_translation' in existing_df.columns:
            # Find the last non-null translation
            non_null_mask = existing_df['southern_birifor_translation'].notna()
            if non_null_mask.any():
                ...
                df['southern_birifor_translation'] = None
        else:
    ...
    for batch_num in range(total_batches):
        ...
        # Translate the batch
        translations = translate_batch_to_southern_birifor(batch_nouns)
        
        # Assign translations back to dataframe
        for i, translation in enumerate(translations):
            df.loc[batch_start + i, 'southern_birifor_translation'] = translation
    ...
    successful = df['southern_birifor_translation'].notna().sum()
