...
def translate_batch_to_lelemi(nouns):
    """
    Translate a batch of nouns to Lelemi using Gemini API
    """
    try:
        # Create the model
        ...
        prompt = f"""Translate these list of nouns extracted from Ghanaian news articles to Lelemi. Return ONLY the translations in the same numbered format, one per line. Do not add any explanations or extra text.
...
def main():
    ...
    output_file = "nouns_with_lelemi_translations.csv"
    ...
    if Path(output_file).exists():
        ...
        # Check if lelemi_translation column exists
        if 'lelemi_translation' in existing_df.columns:
            # Find the last non-null translation
            non_null_mask = existing_df['lelemi_translation'].notna()
    ...
    for batch_num in range(total_batches):
        ...
        # Translate the batch
        translations = translate_batch_to_lelemi(batch_nouns)
        
        # Assign translations back to dataframe
        for i, translation in enumerate(translations):
            df.loc[batch_start + i, 'lelemi_translation'] = translation
    ...
    successful = df['lelemi_translation'].notna().sum()
