...
def translate_batch_to_chumburung(nouns):
    """
    Translate a batch of nouns to Chumburung using Gemini API
    """
    try:
        # Create the model
        ...
        prompt = f"""Translate these list of nouns extracted from Ghanaian news articles to Chumburung. Return ONLY the translations in the same numbered format, one per line. Do not add any explanations or extra text.
...
def main():
    ...
    output_file = "nouns_with_chumburung_translations.csv"
    ...
    if Path(output_file).exists():
        ...
        # Check if chumburung_translation column exists
        if 'chumburung_translation' in existing_df.columns:
            # Find the last non-null translation
            non_null_mask = existing_df['chumburung_translation'].notna()
            if non_null_mask.any():
                ...
                df['chumburung_translation'] = None
        else:
    ...
    for batch_num in range(total_batches):
        ...
        # Translate the batch
        translations = translate_batch_to_chumburung(batch_nouns)
        
        # Assign translations back to dataframe
        for i, translation in enumerate(translations):
            df.loc[batch_start + i, 'chumburung_translation'] = translation
    ...
    successful = df['chumburung_translation'].notna().sum()
