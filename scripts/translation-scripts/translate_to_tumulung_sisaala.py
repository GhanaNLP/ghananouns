...
def translate_batch_to_tumulung_sisaala(nouns):
    """
    Translate a batch of nouns to Tumulung Sisaala using Gemini API
    """
    try:
        # Create the model
        ...
        prompt = f"""Translate these list of nouns extracted from Ghanaian news articles to Tumulung Sisaala. Return ONLY the translations in the same numbered format, one per line. Do not add any explanations or extra text.
...
def main():
    ...
    output_file = "nouns_with_tumulung_sisaala_translations.csv"
    ...
    if Path(output_file).exists():
        ...
        # Check if tumulung_sisaala_translation column exists
        if 'tumulung_sisaala_translation' in existing_df.columns:
            # Find the last non-null translation
            non_null_mask = existing_df['tumulung_sisaala_translation'].notna()
            if non_null_mask.any():
                ...
                df['tumulung_sisaala_translation'] = None
        else:
    ...
    for batch_num in range(total_batches):
        ...
        # Translate the batch
        translations = translate_batch_to_tumulung_sisaala(batch_nouns)
        
        # Assign translations back to dataframe
        for i, translation in enumerate(translations):
            df.loc[batch_start + i, 'tumulung_sisaala_translation'] = translation
    ...
    successful = df['tumulung_sisaala_translation'].notna().sum()
