...
def translate_batch_to_paasaal(nouns):
    """
    Translate a batch of nouns to Paasaal using Gemini API
    """
    try:
        # Create the model
        ...
        prompt = f"""Translate these list of nouns extracted from Ghanaian news articles to Paasaal. Return ONLY the translations in the same numbered format, one per line. Do not add any explanations or extra text.
...
def main():
    ...
    output_file = "nouns_with_paasaal_translations.csv"
    ...
    if Path(output_file).exists():
        ...
        # Check if paasaal_translation column exists
        if 'paasaal_translation' in existing_df.columns:
            # Find the last non-null translation
            non_null_mask = existing_df['paasaal_translation'].notna()
    ...
    for batch_num in range(total_batches):
        ...
        # Translate the batch
        translations = translate_batch_to_paasaal(batch_nouns)
        
        # Assign translations back to dataframe
        for i, translation in enumerate(translations):
            df.loc[batch_start + i, 'paasaal_translation'] = translation
    ...
    successful = df['paasaal_translation'].notna().sum()
