...
def translate_batch_to_avatime(nouns):
    """
    Translate a batch of nouns to Avatime using Gemini API
    """
    try:
        # Create the model
        ...
        prompt = f"""Translate these list of nouns extracted from Ghanaian news articles to Avatime. Return ONLY the translations in the same numbered format, one per line. Do not add any explanations or extra text.
...
def main():
    ...
    output_file = "nouns_with_avatime_translations.csv"
    ...
    if Path(output_file).exists():
        ...
        # Check if avatime_translation column exists
        if 'avatime_translation' in existing_df.columns:
            # Find the last non-null translation
            non_null_mask = existing_df['avatime_translation'].notna()
    ...
    for batch_num in range(total_batches):
        ...
        # Translate the batch
        translations = translate_batch_to_avatime(batch_nouns)
        
        # Assign translations back to dataframe
        for i, translation in enumerate(translations):
            df.loc[batch_start + i, 'avatime_translation'] = translation
    ...
    successful = df['avatime_translation'].notna().sum()
